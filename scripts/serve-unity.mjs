...
import {hostedApi} from './hosted-api.mjs';
...
http.createServer(async(req,res)=>{
 try{
  if(process.env.DIVE_HOSTED_API==='1'&&req.url.startsWith('/api/')){await hostedApi(req,res);return;}
...
}).listen(Number(process.env.PORT||5186),'127.0.0.1',()=>console.log('DIVE Unity preview: http://127.0.0.1:'+(process.env.PORT||5186)+'/'));
